        ...
        with trange(self.max_iter) as t:
            for i in t:
                ...
                if len(self.train_loss_list) > 0:
                    n = len(self.train_loss_list)
                    last_loss = self.train_loss_list[n-1]
                t.set_postfix(loss=last_loss)
                self.train_step()
...
def load():
    ...
    x_train = np.array([x for x, t in train])
    t_train = np.array([t for x, t in train])

    x_test = np.array([x for x, t in test])
    t_test = np.array([t for x, t in test])
...
class DeepConvNet:
    ...
    def __init__(self, input_dim=(1, 28, 28),
        ...
        self.params['W7'] = weight_init_scales[6] * \
            np.random.randn(64*8*5, hidden_size)
...
# network = DeepConvNet()
network = DeepConvNet(input_dim=(1,), output_size=16)
